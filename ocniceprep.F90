program ocniceprep
  ...
  mesh_src = trim(griddir)//fsrc(2:4)//'mesh.'//trim(fsrc)//'.nc'
  mesh_dst = trim(griddir)//fdst(2:4)//'mesh.'//trim(fdst)//'.nc'
  ...
  gridfile = trim(griddir)//fsrc(2:4)//'tripole.'//trim(fsrc)//'.nc'
  ...
  gridfile = trim(griddir)//fdst(2:4)//'tripole.'//trim(fdst)//'.nc'
  call nf90_err(nf90_open(trim(gridfile), nf90_nowrite, ncid), 'open: '//trim(gridfile))
  call nf90_err(nf90_open(trim(gridfile), nf90_nowrite, ncid), 'open: '//trim(gridfile))
 ...
 ! --------------------------------------------------------
  ...
  if (allocated(bilin2d)) then
     call packarrays(trim(input_file), trim(wgtsdir), cos(angsrc), sin(angsrc), b2d, dims=(/nxt,nyt/),       &
  ...
  if (allocated(bilin3d))then
     call packarrays(trim(input_file), trim(wgtsdir), cos(angsrc), sin(angsrc), b3d, dims=(/nxt,nyt,nlevs/), &
