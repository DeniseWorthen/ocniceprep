...
contains
  ...
  !----------------------------------------------------------
  ! remap a packed field of nflds using a RH
  !----------------------------------------------------------
  subroutine remapRH2d(src_field,dst_field)
    ...
    !do k = 1,size(src_field,2)
    !   print '(a,2g14.7)','src min/max ',minval(srcptr(:,k)), maxval(srcptr(:,k))
    !end do
    call ESMF_FieldRegrid(fldsrc, flddst, routehandle=rh, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return

    !do k = 1,size(dst_field,2)
    !   print '(a,2g14.7)','dst min/max ',minval(dstptr(:,k)), maxval(dstptr(:,k))
    !end do
    dst_field = dstptr
  ...
  !----------------------------------------------------------
  ! remap a packed field of nflds and nlevs using a RH
  !----------------------------------------------------------
